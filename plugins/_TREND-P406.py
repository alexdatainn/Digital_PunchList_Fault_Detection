...
def run(
    ...
    tags: List[str] = ["POIVoltage10mMIN", "POIVoltLowerLimit"]
    ...
    for asset in [site]:
        if (
            ...
            for i, val in enumerate(measures['v_' + asset + '-' + tags[0]]):
                ...
                volt_min = measures['v_' + asset + '-' + tags[0]][timestamp]
                limit = measures['v_' + asset + '-' + tags[1]][timestamp]
                if (
                    (volt_min > lower_filter) and (volt_min < upper_filter)
                    and (volt_min < limit)
                ):
                    occurances[asset].append({
                        "time": timestamp, "volt": volt_min, "limit": limit,
                    })
